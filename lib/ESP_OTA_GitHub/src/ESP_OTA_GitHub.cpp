...
void ESPOTAGitHub::setInsecure(bool insecure)
{
    _useInsecure = insecure;
}
...
bool ESPOTAGitHub::_resolveRedirects()
{
    ...
    while (!isFinalURL)
    {
        ...
        while (client.connected())
        {
            ...
            {
                isFinalURL = false;
                path = response.substring(response.indexOf(' ') + 1);
                path.trim();
                if (path.startsWith("http://") || path.startsWith("https://"))
                {
                    urlDetails_t newURL = _urlDetails(path);
                    proto = newURL.proto;
                    host = newURL.host;
                    port = newURL.port;
                    path = newURL.path;
...
bool ESPOTAGitHub::doUpgrade()
{
    ...
    _lastError = "Unknown HTTP update result";
    return false;
}
